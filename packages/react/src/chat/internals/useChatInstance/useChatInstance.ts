...
export function useChatInstance<Cursor = string>(
  ...
  // When a controlled model was mutated internally, the parent re-renders but
  // may pass the same controlled prop references, so the effect above won't fire.
  // Resync here to restore the parent's authoritative values.
  React.useEffect(() => {
