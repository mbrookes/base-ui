...
<Subtitle>
  Keyboard navigation, landmarks, and screen reader announcements built into the chat components.
</Subtitle>
<Meta
  name="description"
...
Custom interactive content rendered inside a message can participate in the drill-in model with the `useMessageContentTabIndex()` hook, exported from `@base-ui/chat/chat-message-list`:

```tsx
import { useMessageContentTabIndex } from '@base-ui/chat/chat-message-list';
...
```tsx
import { useMessageActionable } from '@base-ui/chat/chat-message-list';

function CustomMessage() {
  const isActionable = useMessageActionable();
  return <div data-actionable={isActionable}>{/* render content */}</div>;
}
```
