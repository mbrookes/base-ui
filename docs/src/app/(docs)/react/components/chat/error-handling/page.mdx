...
<Subtitle>Handle errors raised by adapters, streams, and history loading through a unified error model.</Subtitle>
<Meta
  name="description"
...
```tsx
import { useMessageError } from '@base-ui/react/chat';
...
See [Reconnecting to streams](/react/components/chat/streaming/#reconnecting-to-streams) for details.
...
```tsx
import type { ChatMessage } from '@base-ui/react/chat';
...
- [Streaming](/react/components/chat/streaming/) for stream lifecycle and disconnect handling.
- [Adapters](/react/components/chat/adapters/) for the adapter interface and error propagation details.
